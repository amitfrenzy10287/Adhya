...
export function fetchFacebookEngage() {
  return (dispatch, getState) => new Promise(function(resolve, reject) {
    ...
    fetch(`${API_HOST_SP}/facebook_user_engaged_by?facebook_screen_name=${twitter_screen_name}&_token=${token}`, myInit)
      .then(response => {
        if(response.status >= 400) {
          throw new Error(response.statusText || 'Internal server error');
        }
...
export function fetchTwitterRepliesGot() {
  return (dispatch, getState) => new Promise(function(resolve, reject) {
    ...
    fetch(`${API_HOST_SP}/twitter_tweet_replies_got?twitter_screen_name=${twitter_screen_name}&_token=${token}`, myInit)
      .then(response => {
        if(response.status >= 400) {
          throw new Error(response.statusText || 'Internal server error');
        }
